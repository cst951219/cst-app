...
export class App {
    ...
    async pullFromCloud() {
        if (!this.cloudSyncEnabled || !this.deviceId) return;
        try {
            const res = await fetch(`/.netlify/functions/sync-data?deviceId=${encodeURIComponent(this.deviceId)}`);
            const result = await res.json();
            if (result.success && result.data) {
                const cloudData = result.data;
                // 只更新运动数据（其他数据保留本地的，避免覆盖用户的本地修改）
                if (cloudData.exercise) {
                    const localData = this.storage.exportAll();
                    localData.exercise = cloudData.exercise;
                    this.storage.importAll(localData);
                    console.log('[CST APP] 运动数据已从云端同步');
                }
            }
        } catch (e) {
            console.error('[CST APP] 从云端拉取数据失败:', e);
        }
    }
